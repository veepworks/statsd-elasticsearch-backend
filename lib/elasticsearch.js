...
var flush_stats = function elastic_flush(ts, metrics)
{
  ...
  for (key in metrics.counters) {
    numStats += fm.counters(key, metrics.counters[key], ts, array_counts, elasticPartNames);
  }

  for (key in metrics.timers) {
    numStats += fm.timers(key, metrics.timers[key], ts, array_timers, elasticPartNames);
  }

  if (array_timers.length > 0) {
    for (key in metrics.timer_data) {
      fm.timer_data(key, metrics.timer_data[key], ts, array_timer_data, elasticPartNames);
    }
  }

  for (key in metrics.gauges) {
    numStats += fm.gauges(key, metrics.gauges[key], ts, array_gauges, elasticPartNames);
  }

  for (key in metrics.sets) {
    numStats += fm.gauges(key, metrics.sets[key].size(), ts, array_gauges, elasticPartNames);
    metrics.sets[key].clear()
  }
...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  elasticPartNames      = configEs.partNames        || ['ns', 'grp', 'tgt']
